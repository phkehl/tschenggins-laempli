...
// from https://raw.githubusercontent.com/Spritetm/libesphttpd/master/util/captdns.c
// modified by flipflip
...
//      my_ntohs(&hdr->id), hdr->flags, hdr->rcode, my_ntohs(&hdr->qdcount), my_ntohs(&hdr->ancount), my_ntohs(&hdr->nscount), my_ntohs(&hdr->arcount), length);
    //DEBUG("dns: id=0x%04x flags=0x%02x rcode=0x%02x qcnt=%d ancnt=%d nscount=%d arcount=%d len=%d",
    //  my_ntohs(&hdr->id), hdr->flags, hdr->rcode, my_ntohs(&hdr->qdcount), my_ntohs(&hdr->ancount), my_ntohs(&hdr->nscount), my_ntohs(&hdr->arcount), length);
    ...
    for (i=0; i<my_ntohs(&hdr->qdcount); i++) {
        ...
        DEBUG("dns: query type=0x%04x class=0x%04x for %s", my_ntohs(&qf->type), my_ntohs(&qf->class), buff);
...
//          httpd_printf("Added NS rec to resp. Resp len is %d\n", (rend-reply));
            ...
            strcpy_P(rend, PSTR("http://esp.nonet"));
            rend+=16;
            setn16(&rhdr->ancount, my_ntohs(&rhdr->ancount)+1);
//          httpd_printf("Added NS rec to resp. Resp len is %d\n", (rend-reply));
        }
...
void ICACHE_FLASH_ATTR captdnsInit(void)
{
    ...
    espconn_create(&conn);
    PRINT("dns: listen "IPSTR":%u", IP2STR(udpconn.local_ip), udpconn.local_port);
}
