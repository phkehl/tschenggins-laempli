...
#if ( (USER_HSV2RGB_METHOD == 1) || (USER_HSV2RGB_METHOD == 2) )
...
// ***** classic conversion *****
#if (USER_HSV2RGB_METHOD == 1)


void ICACHE_FLASH_ATTR hsv2rgb(const uint8_t H, const uint8_t S, uint8_t V, uint8_t *R, uint8_t *G, uint8_t *B)
{
    HSV2RGB_CLASSIC(H, S, V, *R, *G, *B);
}

// ***** saturation/value dimming *****
#elif (USER_HSV2RGB_METHOD == 2)
...
#else
#  error Illegal value for USER_HSV2RGB_METHOD!
#endif
